...
module.exports = (mongo) => {
  const getViewCountForPostId = async postId => {
    ...
    return {
      postId,
      count: await viewsRepository(mongo).getViewCountByParams({ postId })
    }
  }
  ...
  const getViews = async params => {
    ...
    return viewsRepository(mongo).getViewsByParams(params)
  }
  ...
  const addView = async params => {
    ...
    const views = await viewsRepository(mongo).addView(params)
